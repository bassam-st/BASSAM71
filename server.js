...
const localCatalogPath = path.join("prices","fallback_prices_catalog.json");
let CATALOG = [];
let fuse = null;
...
async function loadRemoteCatalog() {
  if (!PRICE_CATALOG_URL) return null;
  try {
    const r = await fetch(PRICE_CATALOG_URL, { cache: "no-store" });
...
async function loadCatalog() {
  ...
  CATALOG = remote && remote.length ? remote : loadLocalCatalog();
...
function detectIntent(text) {
  ...
  if (q.includes("رول")) return { kind: "rolls" };
  if (q.includes("طن") || q.includes("حديد")) return { kind: "kgOrTon" };
  return null;
}
...
app.post("/api/ask", async (req,res)=>{
  try {
    ...
    return res.json({
      reply: text,
      openCalcUrl: `${CALCULATOR_URL}?price=${encodeURIComponent(r.usd)}&qty=1&ratePct=${r.ratePct}`
    });
  } catch(e) {
    console.error(e);
